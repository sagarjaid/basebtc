...
const cta: JSX.Element = (
  ...
  <button className=' w-fit flex gap-3 items-center px-4 py-2 text-gray-900 font-semibold border border-gray-900 rounded-lg'>
    <span> Home</span>
  </button>
);
                  ...
                  <div className='relative grid gap-2 bg-base-100 p-2 overflow-hidden'>
                    {categories.map((category) => (
                      <div
                        ...
                        onClick={() => close()}>
                        <Link
                          className='block text-left p-3 -m-1 cursor-pointer hover: rounded-box duration-200'
  ...
  return (
    <header className=''>
      ...
      <div className={`relative z-50 bg-white ${isOpen ? '' : 'hidden'}`}>
        <div
          className={`fixed inset-y-0 right-0 z-10 w-full bg-white px-4 overflow-y-auto  sm:max-w-sm sm:ring-1 sm:ring-neutral/10 transform origin-right transition ease-in-out duration-300`}>
